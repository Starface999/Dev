...
$(document).ready(function(){
	...
	$("#postEventForm").submit(function(e){
		...
		var image = $('#image').val();
		var addressOrCoordinates;
		var rawLat = Number(latitude);
		var rawLong = Number(longitude);
	
		if ((latitude + longitude).length>0) {
			addressOrCoordinates = latitude + ", " + longitude;
		}
		else if (addressStreet.length>0 && addressCity.length>0 && addressState.length>0 && addressZip.length>0) {
			addressOrCoordinates = addressStreet + ", " + addressCity + ", " + addressState + " " + addressZip;
		}
		else {
			addressOrCoordinates = "this is a super-secret event with no location posted!";
		}

		console.log(addressOrCoordinates);

		$("#eventPosts").append("<div class='eventPosted' id='" + locations.length + "'><div><strong>" + nameOfEvent + "</strong><a href='#' class='delete pull-right'><sup><i>delete</i></sup></a></div><br><div id='eventDate'>" + date + "  |  " + time + "</div><br><div>Location:</div><div id='eventLocation'>" + addressOrCoordinates + "</div><div><img class='eventImage' src='" + image + "'></div><br><div>Description:</div><div>" + description + "<p><HR></p><br><div>");
